...
namespace SSD_Components
{
	...
	public:
		...
		void PrepareSuspend(int device_no)
		{
			SuspendedCommand = ActiveCommand;
			RemainingExecTime = Expected_finish_time - MQsim_Simulator[device_no]->Time();
		...
		void PrepareResume(int device_no)
		{
			ActiveCommand = SuspendedCommand;
			Expected_finish_time = MQsim_Simulator[device_no]->Time() + RemainingExecTime;
	...
	public:
		...
		void PrepareSuspend(int device_no) { HasSuspend = true; No_of_active_dies = 0; }
		void PrepareResume(int device_no) { HasSuspend = false; }
	};
	...
	private:
		...
		static void handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command, int id);
