...
namespace NVM
{
	namespace FlashMemory
	{
		...
		public:
			...
			void StartCMDXfer(int device_no)
			{
				this->lastTransferStart = MQsim_Simulator[device_no]->Time();
			}
			void StartCMDDataInXfer(int device_no)
			{
				this->lastTransferStart = MQsim_Simulator[device_no]->Time();
			}
			void StartDataOutXfer(int device_no)
			{
				this->lastTransferStart = MQsim_Simulator[device_no]->Time();
			}
			void EndCMDXfer(Flash_Command* command,int device_no)//End transferring write data to the Flash chip
			{
				this->STAT_totalXferTime += (MQsim_Simulator[device_no]->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (MQsim_Simulator[device_no]->Time() - lastTransferStart);
				this->Dies[command->Address[0].DieID]->STAT_TotalXferTime += (MQsim_Simulator[device_no]->Time() - lastTransferStart);

				start_command_execution(command);

				this->lastTransferStart = INVALID_TIME;
			}
			void EndCMDDataInXfer(Flash_Command* command,int device_no)//End transferring write data of a group of multi-plane transactions to the Flash chip
			{
				this->STAT_totalXferTime += (MQsim_Simulator[device_no]->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (MQsim_Simulator[device_no]->Time() - lastTransferStart);
			...
			void EndDataOutXfer(Flash_Command* command,int device_no)
			{
				this->STAT_totalXferTime += (MQsim_Simulator[device_no]->Time() - this->lastTransferStart);
				if (this->idleDieNo != die_no)
					STAT_totalOverlappedXferExecTime += (MQsim_Simulator[device_no]->Time() - lastTransferStart);
			...
			typedef void(*ChipReadySignalHandlerType) (Flash_Chip* targetChip, Flash_Command* command, int);
