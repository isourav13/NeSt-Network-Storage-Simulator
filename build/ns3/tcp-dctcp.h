...
class TCPDctcpTracker
{
public:
	double alpha;
	uint32_t app_id;
	double time_in_microsecond;

	uint32_t prev_wnd;
	uint32_t cwnd;

	double target;
	TCPDctcpTracker(double alpha, uint32_t app_id, double time, uint32_t prev_wnd, uint32_t cwnd, double target);
	TCPDctcpTracker();
...
public:
  ...
  static double *m_qulity_factor;
  static double *m_qulity_factor_response;


  static double threshold_low;

  static double threshold_mid;

  static double threshold_high;

  static double tcp_gamma;

  static bool *m_assured_service;

  uint32_t m_ackcount;

  int64_t m_timelastack;

  double m_throughput;

  uint32_t m_ecnackcount;

  //TCP Friendliness
  uint32_t m_reductionCount;

  bool interferance_flow;
  ...
  static TCPDctcpTracker *m_dctcp_tracker;
  static uint32_t tracker_index;

  static double bottleneck_bandwidth;
