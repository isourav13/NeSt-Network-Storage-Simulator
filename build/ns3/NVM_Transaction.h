...
namespace SSD_Components
{
	...
	{
	public:
		NVM_Transaction(int device_id, stream_id_type stream_id, Transaction_Source_Type source, Transaction_Type type, User_Request* user_request, IO_Flow_Priority_Class::Priority priority_class) :
			Stream_id(stream_id), Source(source), Type(type), UserIORequest(user_request), Priority_class(priority_class), Issue_time(MQsim_Simulator[device_id]->Time()), STAT_execution_time(INVALID_TIME), STAT_transfer_time(INVALID_TIME) {}
        ...
        int transc_device_id; //added by JG
