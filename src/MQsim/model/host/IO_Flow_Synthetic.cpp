																														  ...
																														  seed(seed), generate_aligned_addresses(generate_aligned_addresses), alignment_value(alignment_value)
{
	...
	{
		if (stop_time > 0) {
			if (MQsim_Simulator[device_id]->Time() > stop_time) {
				return NULL;
			}
		...
		request->Arrival_time = MQsim_Simulator[device_id]->Time();
	...
	void IO_Flow_Synthetic::Start_simulation() 
	{
		...
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
			MQsim_Simulator[device_id]->Register_sim_event(device_id,(sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0);
		} else {
			MQsim_Simulator[device_id]->Register_sim_event(device_id,(sim_time_type)1, this, 0, 0);
		}
	}
	...
	void IO_Flow_Synthetic::Execute_simulator_event(MQSimEngine::Sim_Event* event)
	{
		if (generator_type == Utils::Request_Generator_Type::BANDWIDTH) {
			...
			if (req != NULL) {
				...
				MQsim_Simulator[device_id]->Register_sim_event(device_id,MQsim_Simulator[device_id]->Time() + (sim_time_type)random_time_interval_generator->Exponential((double)Average_inter_arrival_time_nano_sec), this, 0, 0);
			}
		} else {
	...
	void IO_Flow_Synthetic::Get_statistics(Utils::Workload_Statistics& stats, LPA_type(*Convert_host_logical_address_to_device_address)(int id, LHA_type lha),
