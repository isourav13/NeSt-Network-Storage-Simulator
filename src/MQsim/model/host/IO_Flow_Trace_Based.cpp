...
Host_IO_Request *IO_Flow_Trace_Based::Generate_next_request()
{
	...
	request->Arrival_time = time_offset + MQsim_Simulator[device_id]->Time();
...
void IO_Flow_Trace_Based::Start_simulation()
{
	...
	Utils::Helper_Functions::Remove_cr(trace_line);
	Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter, current_trace_line);
	MQsim_Simulator[device_id]->Register_sim_event(device_id,std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10), this);
}
...
void IO_Flow_Trace_Based::Execute_simulator_event(MQSimEngine::Sim_Event *)
{
	...
	if (STAT_generated_request_count < total_requests_to_be_generated)
	{
		...
		else
		{
			...
			trace_file.open(trace_file_path);
			replay_counter++;
			time_offset = MQsim_Simulator[device_id]->Time();
			std::getline(trace_file, trace_line);
			Utils::Helper_Functions::Remove_cr(trace_line);
			current_trace_line.clear();
			Utils::Helper_Functions::Tokenize(trace_line, ASCIILineDelimiter, current_trace_line);
		...
		char *pEnd;
		MQsim_Simulator[device_id]->Register_sim_event(device_id,time_offset + std::strtoll(current_trace_line[ASCIITraceTimeColumn].c_str(), &pEnd, 10), this);
	}
}

void IO_Flow_Trace_Based::Get_statistics(Utils::Workload_Statistics &stats, LPA_type (*Convert_host_logical_address_to_device_address)(int id, LHA_type lha),
										 page_status_type (*Find_NVM_subunit_access_bitmap)(int id, LHA_type lha))
{
	...
	while (std::getline(trace_file_temp, trace_line))
	{
		...
		{
			LPA_type device_address = Convert_host_logical_address_to_device_address(device_id, start_LBA);
			page_status_type access_status_bitmap = Find_NVM_subunit_access_bitmap(device_id, start_LBA);
