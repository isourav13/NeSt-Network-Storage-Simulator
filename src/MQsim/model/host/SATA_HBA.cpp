		...
		switch ((HBA_Sim_Events)event->Type)
		{
			case HBA_Sim_Events::CONSUME_IO_REQUEST:
			{
				...
				if(consume_requests.size() > 0) {
					MQsim_Simulator[device_id]->Register_sim_event(device_id, MQsim_Simulator[device_id]->Time() + hba_processing_delay, this, NULL, static_cast<int>(HBA_Sim_Events::CONSUME_IO_REQUEST));
				}
				break;
			}
			case HBA_Sim_Events::SUBMIT_IO_REQUEST:
			{
				...
				if (host_requests.size() > 0) {
					MQsim_Simulator[device_id]->Register_sim_event(device_id,MQsim_Simulator[device_id]->Time() + hba_processing_delay, this, NULL, static_cast<int>(HBA_Sim_Events::SUBMIT_IO_REQUEST));
				}
