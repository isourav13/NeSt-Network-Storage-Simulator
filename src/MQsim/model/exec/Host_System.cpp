...
namespace ns3
{
	...
	Host_System::Host_System(Host_Parameter_Set* parameters, bool preconditioning_required, SSD_Components::Host_Interface_Base* ssd_host_interface, uint32_t id2):
		MQSimEngine::Sim_Object("Host"+std::to_string(id2)), preconditioning_required(preconditioning_required)
	{
	    this->host_id = id2;
		MQsim_Simulator[host_id]->AddObject(this);
		...
		MQsim_Simulator[host_id]->AddObject(this->Link);
		...
		for (uint16_t flow_id = 0; flow_id < parameters->IO_Flow_Definitions.size(); flow_id++) {
			...
			switch (parameters->IO_Flow_Definitions[flow_id]->Type) {
				...
				case Flow_Type::NS3: {
									IO_Flow_Parameter_Set_NS3_Based * flow_param = (IO_Flow_Parameter_Set_NS3_Based*)parameters->IO_Flow_Definitions[flow_id];
									io_flow = new Host_Components::IO_Flow_NS3_Based(this->ID() + ".IO_Flow.NS3." + std::to_string(flow_id), flow_id,
										Utils::Logical_Address_Partitioning_Unit::Start_lha_available_to_flow(flow_id), Utils::Logical_Address_Partitioning_Unit::End_lha_available_to_flow(flow_id),
										FLOW_ID_TO_Q_ID(flow_id), nvme_sq_size, nvme_cq_size,
										flow_param->Priority_Class, ssd_host_interface->GetType(), this->PCIe_root_complex, this->SATA_hba,
										parameters->Enable_ResponseTime_Logging, parameters->ResponseTime_Logging_Period_Length, parameters->Input_file_path + ".IO_Flow.No_" + std::to_string(flow_id) + ".log");
			...
			MQsim_Simulator[host_id]->AddObject(io_flow);
		}
