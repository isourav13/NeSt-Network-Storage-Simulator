...
void IO_Flow_Parameter_Set_NS3_Based::XML_serialize(Utils::XmlWriter& xmlwriter)
{

  std::string tmp = "IO_Flow_Parameter_Set_NS3_Based";
  xmlwriter.Write_open_tag(tmp);
  IO_Flow_Parameter_Set::XML_serialize(xmlwriter);

  std::string attr = "Percentage_To_Be_Executed";
  std::string val = std::to_string(Percentage_To_Be_Executed);
  xmlwriter.Write_attribute_string(attr, val);
...
void IO_Flow_Parameter_Set_NS3_Based::XML_deserialize(rapidxml::xml_node<> *node)
{
  IO_Flow_Parameter_Set::XML_deserialize(node);
    ...
    for (auto param = node->first_node(); param; param = param->next_sibling()) {
      if (strcmp(param->name(), "Relay_Count") == 0) {
        ...
        if (strcmp(val.c_str(), "PICOSECOND") == 0) {
          ...
          PRINT_ERROR("Wrong time unit specified for the NS3 based flow")
        }
      }

    }
  } catch (...) {
    PRINT_ERROR("Error in IO_Flow_Parameter_Set_NS3_Based!")
  }
}
