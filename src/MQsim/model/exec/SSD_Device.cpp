...
namespace ns3
{
SSD_Device *SSD_Device::my_instance[DEVICES]; //Used in static functions

SSD_Device::SSD_Device(Device_Parameter_Set *parameters, std::vector<IO_Flow_Parameter_Set *> *io_flows, uint32_t id2) : MQSimEngine::Sim_Object("SSDDevice"+std::to_string(id2))
{
	SSD_Device *device = this;
    this->device_id = id2;
	my_instance[id2] = device; //used for static functions

    //SSD_Components::ssd_components_device_no = this->device_id;
    //SSD_Components::device_id_ns = this->device_id;
	MQsim_Simulator[device_id]->AddObject(device);
	...
	case NVM::NVM_Type::FLASH:
	{
		...
		case SSD_Components::ONFI_Protocol::NVDDR2:
		{
			...
			for (unsigned int channel_cntr = 0; channel_cntr < parameters->Flash_Channel_Count; channel_cntr++)
			{
				...
				for (unsigned int chip_cntr = 0; chip_cntr < parameters->Chip_No_Per_Channel; chip_cntr++)
				{
					...
					MQsim_Simulator[device_id]->AddObject(chips[chip_cntr]); //Each simulation object (a child of MQSimEngine::Sim_Object) should be added to the engine
				}
			...
			MQsim_Simulator[device_id]->AddObject(device->PHY);
			break;
		}
		default:
		...
		MQsim_Simulator[device_id]->AddObject(ftl);
		device->Firmware = ftl;
		...
		MQsim_Simulator[device_id]->AddObject(tsu);
		ftl->TSU = tsu;
		...
		MQsim_Simulator[device_id]->AddObject(amu);
		...
		gcwl = new SSD_Components::GC_and_WL_Unit_Page_Level(ftl->ID() + ".GCandWLUnit", amu, fbm, tsu, (SSD_Components::NVM_PHY_ONFI *)device->PHY,
															 ...
															 parameters->Seed++);
		MQsim_Simulator[device_id]->AddObject(gcwl);
		...
		MQsim_Simulator[device_id]->AddObject(dcm);
		...
		MQsim_Simulator[device_id]->AddObject(device->Host_interface);
...
LPA_type SSD_Device::Convert_host_logical_address_to_device_address( int id, LHA_type lha)
{
	return my_instance[id]->Firmware->Convert_host_logical_address_to_device_address(lha);
}

page_status_type SSD_Device::Find_NVM_subunit_access_bitmap( int id, LHA_type lha)
{
	return my_instance[id]->Firmware->Find_NVM_subunit_access_bitmap(lha);
}
}
