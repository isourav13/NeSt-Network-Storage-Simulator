...
namespace SSD_Components
{
		...
		eraseReasonableSuspensionTimeForWrite(EraseReasonableSuspensionTimeForWrite), opened_scheduling_reqs(0)
	{
        ...
        if (_my_instance == NULL) {
            _my_instance = new TSU_Base *[DEVICES];
        }
	...
	void TSU_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction, int id)
	{
		//TSU does nothing. The generator of the transaction will handle it.
	}

	void TSU_Base::handle_channel_idle_signal(flash_channel_ID_type channelID, int id)
	{
		for (unsigned int i = 0; i < _my_instance[id]->chip_no_per_channel; i++) {
			//The TSU does not check if the chip is idle or not since it is possible to suspend a busy chip and issue a new command
			_my_instance[id]->process_chip_requests(_my_instance[id]->_NVMController->Get_chip(channelID, _my_instance[id]->Round_robin_turn_of_channel[channelID]), id);
			_my_instance[id]->Round_robin_turn_of_channel[channelID] = (flash_chip_ID_type)(_my_instance[id]->Round_robin_turn_of_channel[channelID] + 1) % _my_instance[id]->chip_no_per_channel;

			//A transaction has been started, so TSU should stop searching for another chip
			if (_my_instance[id]->_NVMController->Get_channel_status(channelID) == BusChannelStatus::BUSY) {
				break;
			}
		}
	}
	
	void TSU_Base::handle_chip_idle_signal(NVM::FlashMemory::Flash_Chip* chip, int id)
	{
		if (_my_instance[id]->_NVMController->Get_channel_status(chip->ChannelID) == BusChannelStatus::IDLE) {
			_my_instance[id]->process_chip_requests(chip, id);
		}
	}
			...
			for (Flash_Transaction_Queue::iterator it = sourceQueue1->begin(); it != sourceQueue1->end();)
			{
				if (transaction_is_ready(*it) && (*it)->Address.DieID == dieID && !(planeVector & 1 << (*it)->Address.PlaneID))
				{
					...
					if (planeVector == 0 || (*it)->Address.PageID == pageID)
					{
						...
						DEBUG(issueCntr++ << ": " << MQsim_Simulator[device_id]->Time() <<" Issueing Transaction - Type:" << TRTOSTR((*it)) << ", PPA:" << (*it)->PPA << ", LPA:" << (*it)->LPA << ", Channel: " << (*it)->Address.ChannelID << ", Chip: " << (*it)->Address.ChipID);
