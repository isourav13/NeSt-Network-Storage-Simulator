	...
	void GC_and_WL_Unit_Page_Level::Check_gc_required(const unsigned int free_block_pool_size, const NVM::FlashMemory::Physical_Page_Address& plane_address)
	{
		if (free_block_pool_size < block_pool_gc_threshold) {
			...
			if (block_manager->Can_execute_gc_wl(gc_candidate_address)) {
				...
				NVM_Transaction_Flash_ER* gc_erase_tr = new NVM_Transaction_Flash_ER(device_id,Transaction_Source_Type::GC_WL, pbke->Blocks[gc_candidate_block_id].Stream_id, gc_candidate_address);
				...
				if (block->Current_page_write_index - block->Invalid_page_count > 0) {
					...
					for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
						if (block_manager->Is_page_valid(block, pageID)) {
							...
							if (use_copyback) {
								gc_write = new NVM_Transaction_Flash_WR(device_id,Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
								...
								gc_read = new NVM_Transaction_Flash_RD(device_id,Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
