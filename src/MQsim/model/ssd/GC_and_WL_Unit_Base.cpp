...
namespace SSD_Components
{
		...
		dynamic_wearleveling_enabled(dynamic_wearleveling_enabled), static_wearleveling_enabled(static_wearleveling_enabled), static_wearleveling_threshold(static_wearleveling_threshold)
	{
        ...
        if (_my_instance == NULL) {
            _my_instance = new GC_and_WL_Unit_Base *[DEVICES];
        }
	...
	void GC_and_WL_Unit_Base::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction, int id)
	{
		PlaneBookKeepingType* pbke = &(_my_instance[id]->block_manager->plane_manager[transaction->Address.ChannelID][transaction->Address.ChipID][transaction->Address.DieID][transaction->Address.PlaneID]);
			...
			case Transaction_Source_Type::CACHE:
				switch (transaction->Type)
				{
					case Transaction_Type::READ:
						_my_instance[id]->block_manager->Read_transaction_serviced(transaction->Address);
						break;
					case Transaction_Type::WRITE:
						_my_instance[id]->block_manager->Program_transaction_serviced(transaction->Address);
						break;
					default:
						PRINT_ERROR("Unexpected situation in the GC_and_WL_Unit_Base function!")
				}
				if (_my_instance[id]->block_manager->Block_has_ongoing_gc_wl(transaction->Address)) {
					if (_my_instance[id]->block_manager->Can_execute_gc_wl(transaction->Address)) {
						...
						_my_instance[id]->tsu->Prepare_for_transaction_submit();
						NVM_Transaction_Flash_ER* gc_wl_erase_tr = new NVM_Transaction_Flash_ER(transaction->transc_device_id,Transaction_Source_Type::GC_WL, block->Stream_id, gc_wl_candidate_address);
						...
						if (block->Current_page_write_index - block->Invalid_page_count > 0) {
							...
							for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
								if (_my_instance[id]->block_manager->Is_page_valid(block, pageID)) {
									Stats::Total_page_movements_for_gc++;
									gc_wl_candidate_address.PageID = pageID;
									if (_my_instance[id]->use_copyback) {
										gc_wl_write = new NVM_Transaction_Flash_WR(transaction->transc_device_id,Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance[id]->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance[id]->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), NULL, 0, NULL, 0, INVALID_TIME_STAMP);
										gc_wl_write->ExecutionMode = WriteExecutionModeType::COPYBACK;
										_my_instance[id]->tsu->Submit_transaction(gc_wl_write);
									} else {
										gc_wl_read = new NVM_Transaction_Flash_RD(transaction->transc_device_id,Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance[id]->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
											NO_LPA, _my_instance[id]->address_mapping_unit->Convert_address_to_ppa(gc_wl_candidate_address), gc_wl_candidate_address, NULL, 0, NULL, 0, INVALID_TIME_STAMP);
										gc_wl_write = new NVM_Transaction_Flash_WR(transaction->transc_device_id,Transaction_Source_Type::GC_WL, block->Stream_id, _my_instance[id]->sector_no_per_page * SECTOR_SIZE_IN_BYTE,
										...
										_my_instance[id]->tsu->Submit_transaction(gc_wl_read);//Only the read transaction would be submitted. The Write transaction is submitted when the read transaction is finished and the LPA of the target page is determined
									}
		...
		switch (transaction->Type) {
			case Transaction_Type::READ:
			{
				...
				if (pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data) {
					_my_instance[id]->address_mapping_unit->Get_translation_mapping_info_for_gc(transaction->Stream_id, (MVPN_type)transaction->LPA, mppa, page_status_bitmap);
					...
					if (mppa == transaction->PPA) {
						...
						_my_instance[id]->address_mapping_unit->Allocate_new_page_for_gc(((NVM_Transaction_Flash_RD*)transaction)->RelatedWrite, pbke->Blocks[transaction->Address.BlockID].Holds_mapping_data);
						_my_instance[id]->tsu->Submit_transaction(((NVM_Transaction_Flash_RD*)transaction)->RelatedWrite);
						_my_instance[id]->tsu->Schedule();
					} else {
						PRINT_ERROR("Inconsistency found when moving a page for GC/WL!")
					}
				} else {
					_my_instance[id]->address_mapping_unit->Get_data_mapping_info_for_gc(transaction->Stream_id, transaction->LPA, ppa, page_status_bitmap);
			...
			case Transaction_Type::WRITE:
				if (pbke->Blocks[((NVM_Transaction_Flash_WR*)transaction)->RelatedErase->Address.BlockID].Holds_mapping_data) {
					_my_instance[id]->address_mapping_unit->Remove_barrier_for_accessing_mvpn(transaction->Stream_id, (MVPN_type)transaction->LPA);
					DEBUG(MQsim_Simulator[device_id]->Time() << ": MVPN=" << (MVPN_type)transaction->LPA << " unlocked!!");
				} else {
					_my_instance[id]->address_mapping_unit->Remove_barrier_for_accessing_lpa(transaction->Stream_id, transaction->LPA);
					DEBUG(MQsim_Simulator[device_id]->Time() << ": LPA=" << (MVPN_type)transaction->LPA << " unlocked!!");
				}
			...
			case Transaction_Type::ERASE:
				...
				_my_instance[id]->block_manager->Add_erased_block_to_pool(transaction->Address);
				_my_instance[id]->block_manager->GC_WL_finished(transaction->Address);
				if (_my_instance[id]->check_static_wl_required(transaction->Address)) {
					_my_instance[id]->run_static_wearleveling(transaction->Address);
				}
				_my_instance[id]->address_mapping_unit->Start_servicing_writes_for_overfull_plane(transaction->Address);//Must be inovked after above statements since it may lead to flash page consumption for waiting program transactions

				if (_my_instance[id]->Stop_servicing_writes(transaction->Address, id)) {
					_my_instance[id]->Check_gc_required(pbke->Get_free_block_pool_size(), transaction->Address);
				}
				break;
			} //switch (transaction->Type)
	}
	...
	bool GC_and_WL_Unit_Base::Stop_servicing_writes(const NVM::FlashMemory::Physical_Page_Address& plane_address, int id)
	{
		PlaneBookKeepingType* pbke = &(_my_instance[id]->block_manager->plane_manager[plane_address.ChannelID][plane_address.ChipID][plane_address.DieID][plane_address.PlaneID]);
	...
	void GC_and_WL_Unit_Base::run_static_wearleveling(const NVM::FlashMemory::Physical_Page_Address plane_address)
	{
		...
		if (block_manager->Can_execute_gc_wl(wl_candidate_address)) {//If there are ongoing requests targeting the candidate block, the gc execution should be postponed
			...
			NVM_Transaction_Flash_ER* wl_erase_tr = new NVM_Transaction_Flash_ER(device_id,Transaction_Source_Type::GC_WL, pbke->Blocks[wl_candidate_block_id].Stream_id, wl_candidate_address);
			if (block->Current_page_write_index - block->Invalid_page_count > 0) {//If there are some valid pages in block, then prepare flash transactions for page movement
				...
				for (flash_page_ID_type pageID = 0; pageID < block->Current_page_write_index; pageID++) {
					if (block_manager->Is_page_valid(block, pageID)) {
						...
						if (use_copyback) {
							wl_write = new NVM_Transaction_Flash_WR(device_id,Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
							...
							wl_read = new NVM_Transaction_Flash_RD(device_id,Transaction_Source_Type::GC_WL, block->Stream_id, sector_no_per_page * SECTOR_SIZE_IN_BYTE,
