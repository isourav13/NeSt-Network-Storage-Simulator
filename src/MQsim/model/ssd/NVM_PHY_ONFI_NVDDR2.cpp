...
namespace SSD_Components {
		...
		: NVM_PHY_ONFI(id, ChannelCount, chip_no_per_channel, DieNoPerChip, PlaneNoPerDie), channels(channels)
	{
        ...
        if(std::isdigit(this->ID()[10])==0)
            device_no = this->ID()[9] - '0';
        else
            device_no = std::stoi(this->ID().substr(9,2));
        ...
        if (_my_instance == NULL) {
            _my_instance = new NVM_PHY_ONFI_NVDDR2 *[DEVICES];
        }
		_my_instance[device_no] = this;
	}
		...
		* then something illegarl is happening*/
		...
		if (!dieBKE->Free) {
			if (transaction_list.front()->SuspendRequired) {
				...
				dieBKE->PrepareSuspend(device_no);
				if (chipBKE->OngoingDieCMDTransfers.size()) {
					chipBKE->PrepareSuspend(device_no);
				}
			} else {
				PRINT_ERROR("Read suspension is not supported!")
			}
		...
		switch (transaction_list.front()->Type) {
			case Transaction_Type::READ:
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					targetChip->StartCMDXfer(device_no);
					chipBKE->Status = ChipStatus::CMD_IN;
					chipBKE->Last_transfer_finish_time = MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()];
					MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->ReadCommandTime[transaction_list.size()], this,
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)transaction_list.front())->ExecutionMode == WriteExecutionModeType::SIMPLE) {
					...
					if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
						targetChip->StartCMDDataInXfer(device_no);
						chipBKE->Status = ChipStatus::CMD_DATA_IN;
						chipBKE->Last_transfer_finish_time = MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time;
						MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->ProgramCommandTime[transaction_list.size()] + data_transfer_time,
			...
			case Transaction_Type::ERASE:
				...
				if (chipBKE->OngoingDieCMDTransfers.size() == 0) {
					targetChip->StartCMDXfer(device_no);
					chipBKE->Status = ChipStatus::CMD_IN;
					chipBKE->Last_transfer_finish_time = MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()];
					MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + suspendTime + target_channel->EraseCommandTime[transaction_list.size()],
	...
	void NVM_PHY_ONFI_NVDDR2::Execute_simulator_event(MQSimEngine::Sim_Event* ev)
	{
		...
		switch ((NVDDR2_SimEventType)ev->Type) {
			case NVDDR2_SimEventType::READ_CMD_ADDR_TRANSFERRED:
				...
				targetChip->EndCMDXfer(dieBKE->ActiveCommand,device_no);
				for (auto tr : dieBKE->ActiveTransactions) {
					tr->STAT_execution_time = dieBKE->Expected_finish_time - MQsim_Simulator[device_no]->Time();
				}
			...
			case NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED:
				...
				targetChip->EndCMDDataInXfer(dieBKE->ActiveCommand,device_no);
				for (auto &tr : dieBKE->ActiveTransactions) {
					tr->STAT_execution_time = dieBKE->Expected_finish_time - MQsim_Simulator[device_no]->Time();
				}
				chipBKE->OngoingDieCMDTransfers.pop();
			...
			case NVDDR2_SimEventType::READ_DATA_TRANSFERRED:
				...
				targetChip->EndDataOutXfer(dieBKE->ActiveCommand, device_no);
		...
		if (WaitingCopybackWrites[channel_id].size() > 0) {
			...
			MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()],
			...
			waitingBKE->Expected_finish_time = MQsim_Simulator[device_no]->Time() + this->channels[channel_id]->ProgramCommandTime[waitingBKE->ActiveTransactions.size()]
	...
	inline void NVM_PHY_ONFI_NVDDR2::handle_ready_signal_from_chip(NVM::FlashMemory::Flash_Chip* chip, NVM::FlashMemory::Flash_Command* command, int id)
	{
		ChipBookKeepingEntry *chipBKE = &(_my_instance[id])->bookKeepingTable[chip->ChannelID][chip->ChipID];
		...
		case CMD_READ_PAGE_MULTIPLANE:
			...
			{
				chipBKE->WaitingReadTXCount++;
				if (_my_instance[id]->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
					_my_instance[id]->transfer_read_data_from_chip(chipBKE, dieBKE, (*it));
				else
				{
					...
					case Transaction_Source_Type::USERIO:
						_my_instance[id]->WaitingReadTX[chip->ChannelID].push_back((*it));
						break;
					case Transaction_Source_Type::GC_WL:
						_my_instance[id]->WaitingGCRead_TX[chip->ChannelID].push_back((*it));
						break;
					case Transaction_Source_Type::MAPPING:
						_my_instance[id]->WaitingMappingRead_TX[chip->ChannelID].push_back((*it));
						break;
					}
				}
			}
		...
		case CMD_READ_PAGE_COPYBACK_MULTIPLANE:
			...
			if (_my_instance[id]->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			{
				...
				for (std::list<NVM_Transaction_Flash*>::iterator it = dieBKE->ActiveTransactions.begin();
					it != dieBKE->ActiveTransactions.end(); it++)
				{
					(*it)->STAT_transfer_time += _my_instance[id]->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()];
				}
				chip->StartCMDXfer(chip->device_id);
				chipBKE->Status = ChipStatus::CMD_IN;
				MQsim_Simulator[chip->device_id]->Register_sim_event(chip->device_id,MQsim_Simulator[chip->device_id]->Time() + _my_instance[id]->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()],
					_my_instance[id], dieBKE, (int)NVDDR2_SimEventType::PROGRAM_COPYBACK_CMD_ADDR_TRANSFERRED);
				chipBKE->OngoingDieCMDTransfers.push(dieBKE);
				_my_instance[id]->channels[chip->ChannelID]->SetStatus(BusChannelStatus::BUSY, chip);

				dieBKE->Expected_finish_time = MQsim_Simulator[chip->device_id]->Time() + _my_instance[id]->channels[chip->ChannelID]->ProgramCommandTime[dieBKE->ActiveTransactions.size()]
				...
				MQsim_Simulator[chip->device_id]->RegisterEvent(MQsim_Simulator[chip->device_id]->Time() + channels[targetChip->ChannelID]->ProgramCommandTime + NVDDR2DataOutTransferTime(targetTransaction->SizeInByte, channels[targetChip->ChannelID]),
			...
			else _my_instance[id]->WaitingCopybackWrites->push_back(dieBKE);
			break;
		...
		case CMD_PROGRAM_PAGE_COPYBACK_MULTIPLANE:
		{
				...
				it != dieBKE->ActiveTransactions.end(); it++, i++)
			{
				...
				_my_instance[id]->broadcastTransactionServicedSignal(*it);
			}
			...
			if (chipBKE->Status == ChipStatus::IDLE)
				if (chipBKE->HasSuspend)
					_my_instance[id]->send_resume_command_to_chip(chip, chipBKE);
			break;
		}
		...
		if (_my_instance[id]->channels[chip->ChannelID]->GetStatus() == BusChannelStatus::IDLE)
			_my_instance[id]->broadcastChannelIdleSignal(chip->ChannelID);
		else if (chipBKE->Status == ChipStatus::IDLE)
			_my_instance[id]->broadcastChipIdleSignal(chip);
	}
	...
	inline void NVM_PHY_ONFI_NVDDR2::transfer_read_data_from_chip(ChipBookKeepingEntry* chipBKE, DieBookKeepingEntry* dieBKE, NVM_Transaction_Flash* tr)
	{
		...
		channels[tr->Address.ChannelID]->Chips[tr->Address.ChipID]->StartDataOutXfer(device_no);
		chipBKE->Status = ChipStatus::DATA_OUT;
		MQsim_Simulator[tr->transc_device_id]->Register_sim_event(device_no,MQsim_Simulator[tr->transc_device_id]->Time() + NVDDR2DataOutTransferTime(tr->Data_and_metadata_size_in_byte, channels[tr->Address.ChannelID]),
	...
	void NVM_PHY_ONFI_NVDDR2::perform_interleaved_cmd_data_transfer(NVM::FlashMemory::Flash_Chip* chip, DieBookKeepingEntry* bookKeepingEntry)
	{
		...
		{
			case Transaction_Type::READ:
				chip->StartCMDXfer(device_no);
				bookKeepingTable[chip->ChannelID][chip->ChipID].Status = ChipStatus::CMD_IN;
				MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + bookKeepingEntry->DieInterleavedTime,
			...
			case Transaction_Type::WRITE:
				if (((NVM_Transaction_Flash_WR*)bookKeepingEntry->ActiveTransactions.front())->RelatedRead == NULL) {
					chip->StartCMDDataInXfer(device_no);
					...
					chip->StartCMDXfer(device_no);
					bookKeepingTable[chip->ChannelID][chip->ChipID].Status = ChipStatus::CMD_IN;
					MQsim_Simulator[device_no]->Register_sim_event(device_no,MQsim_Simulator[device_no]->Time() + bookKeepingEntry->DieInterleavedTime, this,
	...
	inline void NVM_PHY_ONFI_NVDDR2::send_resume_command_to_chip(NVM::FlashMemory::Flash_Chip* chip, ChipBookKeepingEntry* chipBKE)
	{
		...
		for (unsigned int i = 0; i < die_no_per_chip; i++) {
			...
			dieBKE->PrepareResume(device_no);
			chipBKE->PrepareResume(device_no);
