...
protected:
	...
	static void handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash *transaction, int id);
	static void handle_channel_idle_signal(flash_channel_ID_type, int id);
	static void handle_chip_idle_signal(NVM::FlashMemory::Flash_Chip *, int id);
	int opened_scheduling_reqs;
	void process_chip_requests(NVM::FlashMemory::Flash_Chip* chip, int id)
	{
		if (!_my_instance[id]->service_read_transaction(chip)) {
			if (!_my_instance[id]->service_write_transaction(chip)) {
				_my_instance[id]->service_erase_transaction(chip);
			}
		}
	}
