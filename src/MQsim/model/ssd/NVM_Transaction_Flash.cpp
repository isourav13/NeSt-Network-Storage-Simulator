...
{
	NVM_Transaction_Flash::NVM_Transaction_Flash(int device_id, Transaction_Source_Type source, Transaction_Type type, stream_id_type stream_id,
		...
		NVM_Transaction(device_id, stream_id, source, type, user_request, priority_class),
	...
	NVM_Transaction_Flash::NVM_Transaction_Flash(int device_id, Transaction_Source_Type source, Transaction_Type type, stream_id_type stream_id,
		...
		NVM_Transaction(device_id, stream_id, source, type, user_request, priority_class), Data_and_metadata_size_in_byte(data_size_in_byte), LPA(lpa), PPA(ppa), Address(address), Physical_address_determined(false)
	{
        transc_device_id = device_id;
	}
}
