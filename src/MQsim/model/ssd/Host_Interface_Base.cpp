...
namespace SSD_Components
{
		...
		sectors_per_page(sectors_per_page), cache(cache)
	{
        ...
        if (_my_instance == NULL) {
            _my_instance = new Host_Interface_Base *[DEVICES];
        }
	...
	void Host_Interface_Base::Send_write_message_to_host(uint64_t addresss, void* message, unsigned int message_size)
	{
		...
		pcie_message->Destination = Host_Components::PCIe_Destination_Type::HOST;
		pcie_message->Address = addresss;
		COPYDATA(pcie_message->Payload, message, pcie_message->Payload_size,device_id);
	...
	void Input_Stream_Manager_Base::Update_transaction_statistics(NVM_Transaction* transaction)
	{
		switch (transaction->Type)
		{
			case Transaction_Type::READ:
				...
				this->input_streams[transaction->Stream_id]->STAT_sum_of_read_transactions_waiting_time += (MQsim_Simulator[device_id]->Time() - transaction->Issue_time) - transaction->STAT_execution_time - transaction->STAT_transfer_time;
				break;
			case Transaction_Type::WRITE:
				...
				this->input_streams[transaction->Stream_id]->STAT_sum_of_write_transactions_waiting_time += (MQsim_Simulator[device_id]->Time() - transaction->Issue_time) - transaction->STAT_execution_time - transaction->STAT_transfer_time;
				break;
			default:
				break;
		}
	...
	Request_Fetch_Unit_Base::Request_Fetch_Unit_Base(Host_Interface_Base* host_interface) :
		host_interface(host_interface)
	{
        device_id = host_interface->device_id;
	}
}
}
