...
namespace SSD_Components
{
	...
	inline void AddressMappingDomain::Update_mapping_info(int device_no, const bool ideal_mapping, const stream_id_type stream_id, const LPA_type lpa, const PPA_type ppa, const page_status_type page_status_bitmap)
	{
		if (ideal_mapping) {
			GlobalMappingTable[lpa].PPA = ppa;
			GlobalMappingTable[lpa].WrittenStateBitmap = page_status_bitmap;
			GlobalMappingTable[lpa].TimeStamp = CurrentTimeStamp(device_no);
		} else {
			...
			Block_no_per_plane, Page_no_per_block, SectorsPerPage, PageSizeInByte, Overprovisioning_ratio, sharing_mode, fold_large_addresses)
	{
        ...
        if (_my_instance == NULL) {
            _my_instance = new Address_Mapping_Unit_Page_Level *[DEVICES];
        }
	...
	void Address_Mapping_Unit_Page_Level::Store_mapping_table_on_flash_at_start()
	{
		...
		NVM_Transaction_Flash_WR* dummy_tr = new NVM_Transaction_Flash_WR(device_id,Transaction_Source_Type::MAPPING, 0, 0,
	...
	*/
	bool Address_Mapping_Unit_Page_Level::translate_lpa_to_ppa(stream_id_type streamID, NVM_Transaction_Flash* transaction)
	{
		...
		if (transaction->Type == Transaction_Type::READ) {
			...
			if (ftl->GC_and_WL_Unit->Stop_servicing_writes(transaction->Address, device_id)){
				return false;
			}
	...
	void Address_Mapping_Unit_Page_Level::Allocate_new_page_for_gc(NVM_Transaction_Flash_WR* transaction, bool is_translation_page)
	{
		if (is_translation_page) {
				...
				if (!domains[transaction->Stream_id]->CMT->Check_free_slot_availability()) {
					...
					if (evictedItem.Dirty) {
						...
						* after eviction happens.*/
						...
						if (domains[transaction->Stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp > CurrentTimeStamp(device_id)) {
							throw std::logic_error("Unexpected situation occured in handling GMT!");
						}
						domains[transaction->Stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp = CurrentTimeStamp(device_id);
			...
			if (domains[stream_id]->Mapping_entry_accessible(ideal_mapping_table, stream_id, transaction->LPA)) {
				...
				Stats::writeTR_CMT_hits++;
				Stats::writeTR_CMT_hits_per_stream[stream_id]++;
				domains[stream_id]->Update_mapping_info(device_id,ideal_mapping_table, stream_id, transaction->LPA, transaction->PPA, transaction->write_sectors_bitmap);
				...
				if (!domains[stream_id]->CMT->Check_free_slot_availability()) {
					...
					if (evictedItem.Dirty) {
						...
						* after eviction happens.*/
						...
						if (domains[stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp > CurrentTimeStamp(device_id))
							throw std::logic_error("Unexpected situation occured in handling GMT!");
						domains[stream_id]->GlobalMappingTable[evicted_lpa].TimeStamp = CurrentTimeStamp(device_id);
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_user_write(NVM_Transaction_Flash_WR* transaction, bool is_for_gc)
	{
		...
		if (old_ppa == NO_PPA)  /*this is the first access to the logical page*/
		{
			...
			if (is_for_gc) {
				...
				if (status_intersection == prev_page_status) {
					...
					NVM_Transaction_Flash_RD *update_read_tr = new NVM_Transaction_Flash_RD(device_id,transaction->Source, transaction->Stream_id,
		...
		* may decide to move a page that is just invalidated.*/
		...
		domain->Update_mapping_info(device_id,ideal_mapping_table, transaction->Stream_id, transaction->LPA, transaction->PPA,
	...
	void Address_Mapping_Unit_Page_Level::allocate_page_in_plane_for_translation_write(NVM_Transaction_Flash* transaction, MVPN_type mvpn, bool is_for_gc)
	{
		...
		domain->GlobalTranslationDirectory[mvpn].TimeStamp = CurrentTimeStamp(device_id);
	}
	...
	PPA_type Address_Mapping_Unit_Page_Level::online_create_entry_for_reads(LPA_type lpa, const stream_id_type stream_id, NVM::FlashMemory::Physical_Page_Address& read_address, uint64_t read_sectors_bitmap)
	{
		...
		domain->Update_mapping_info(device_id,ideal_mapping_table, stream_id, lpa, ppa, read_sectors_bitmap);
			...
			if (!domain->CMT->Check_free_slot_availability()) {
				...
				if (evictedItem.Dirty) {
					...
					* after eviction happens.*/
					...
					if (domain->GlobalMappingTable[evicted_lpa].TimeStamp > CurrentTimeStamp(device_id))
						throw std::logic_error("Unexpected situation occurred in handling GMT!");
					domain->GlobalMappingTable[evicted_lpa].TimeStamp = CurrentTimeStamp(device_id);
					generate_flash_writeback_request_for_mapping_data(stream_id, evicted_lpa);
				}
			}
			...
			if (!domain->CMT->Check_free_slot_availability()) {
				...
				if (evictedItem.Dirty) {
					...
					* after eviction happens.*/
					...
					if (domain->GlobalMappingTable[evicted_lpa].TimeStamp > CurrentTimeStamp(device_id))
						throw std::logic_error("Unexpected situation occured in handling GMT!");
					domain->GlobalMappingTable[lpa].TimeStamp = CurrentTimeStamp(device_id);
					generate_flash_writeback_request_for_mapping_data(stream_id, evicted_lpa);
				}
			}
			domain->CMT->Reserve_slot_for_lpn(stream_id, lpa);
	...
	void Address_Mapping_Unit_Page_Level::generate_flash_writeback_request_for_mapping_data(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (is_mvpn_locked_for_gc(stream_id, mvpn)) {
			...
			if (mppn != NO_MPPN) {
				readTR = new NVM_Transaction_Flash_RD(device_id,Transaction_Source_Type::MAPPING, stream_id, read_size,
					mvpn, mppn, NULL, mvpn, NULL, readSectorsBitmap, CurrentTimeStamp(device_id));
			...
			NVM_Transaction_Flash_WR* writeTR = new NVM_Transaction_Flash_WR(device_id,Transaction_Source_Type::MAPPING, stream_id, SECTOR_SIZE_IN_BYTE * sector_no_per_page,
				mvpn, mppn, NULL, mvpn, readTR, (((page_status_type)0x1) << sector_no_per_page) - 1, CurrentTimeStamp(device_id));
	...
	void Address_Mapping_Unit_Page_Level::generate_flash_read_request_for_mapping_data(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (is_mvpn_locked_for_gc(stream_id, mvpn)) {
			...
			NVM_Transaction_Flash_RD* readTR = new NVM_Transaction_Flash_RD(device_id,Transaction_Source_Type::MAPPING, stream_id,
					SECTOR_SIZE_IN_BYTE, NO_LPA, NO_PPA, NULL, mvpn, ((page_status_type)0x1) << sector_no_per_page, CurrentTimeStamp(device_id));
	...
	inline void Address_Mapping_Unit_Page_Level::handle_transaction_serviced_signal_from_PHY(NVM_Transaction_Flash* transaction, int id)
	{
		...
		if (_my_instance[id]->ideal_mapping_table){
		...
		if (transaction->Type == Transaction_Type::WRITE) {
			_my_instance[id]->domains[transaction->Stream_id]->DepartingMappingEntries.erase((MVPN_type)((NVM_Transaction_Flash_WR*)transaction)->Content);
		} else {
			...
			* (stored on flash) with those updated entries that are evicted from CMT*/
			...
			_my_instance[id]->ftl->TSU->Prepare_for_transaction_submit();
			MVPN_type mvpn = (MVPN_type)((NVM_Transaction_Flash_RD*)transaction)->Content;
			std::multimap<MVPN_type, LPA_type>::iterator it = _my_instance[id]->domains[transaction->Stream_id]->ArrivingMappingEntries.find(mvpn);
			while (it != _my_instance[id]->domains[transaction->Stream_id]->ArrivingMappingEntries.end()) {
				if ((*it).first == mvpn) {
					...
					if (_my_instance[id]->domains[transaction->Stream_id]->CMT->Is_slot_reserved_for_lpn_and_waiting(transaction->Stream_id, lpa)) {
						_my_instance[id]->domains[transaction->Stream_id]->CMT->Insert_new_mapping_info(transaction->Stream_id, lpa,
							_my_instance[id]->domains[transaction->Stream_id]->GlobalMappingTable[lpa].PPA,
							_my_instance[id]->domains[transaction->Stream_id]->GlobalMappingTable[lpa].WrittenStateBitmap);
						auto it2 = _my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_read_transactions.find(lpa);
						while (it2 != _my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_read_transactions.end() &&
							(*it2).first == lpa) {
							if (_my_instance[id]->is_lpa_locked_for_gc(transaction->Stream_id, lpa)) {
								_my_instance[id]->manage_user_transaction_facing_barrier(it2->second);
							} else {
								if (_my_instance[id]->translate_lpa_to_ppa(transaction->Stream_id, it2->second)) {
									_my_instance[id]->ftl->TSU->Submit_transaction(it2->second);
								}
								else {
									_my_instance[id]->mange_unsuccessful_translation(it2->second);
								}
							}
							_my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_read_transactions.erase(it2++);
						}
						it2 = _my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_program_transactions.find(lpa);
						while (it2 != _my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_program_transactions.end() &&
							(*it2).first == lpa) {
							if (_my_instance[id]->is_lpa_locked_for_gc(transaction->Stream_id, lpa)) {
								_my_instance[id]->manage_user_transaction_facing_barrier(it2->second);
							} else {
								if (_my_instance[id]->translate_lpa_to_ppa(transaction->Stream_id, it2->second)) {
									...
									if (((NVM_Transaction_Flash_WR*)it2->second)->RelatedRead != NULL) {
										_my_instance[id]->ftl->TSU->Submit_transaction(((NVM_Transaction_Flash_WR*)it2->second)->RelatedRead);
									}
								} else {
									_my_instance[id]->mange_unsuccessful_translation(it2->second);
								}
							}
							_my_instance[id]->domains[transaction->Stream_id]->Waiting_unmapped_program_transactions.erase(it2++);
						}
					}
				} else {
					break;
				}
				_my_instance[id]->domains[transaction->Stream_id]->ArrivingMappingEntries.erase(it++);
			}
			_my_instance[id]->ftl->TSU->Schedule();
		}
	}
	...
	inline void Address_Mapping_Unit_Page_Level::Remove_barrier_for_accessing_lpa(stream_id_type stream_id, LPA_type lpa)
	{
		...
		while (read_tr != domains[stream_id]->Read_transactions_behind_LPA_barrier.end()) {
			handle_transaction_serviced_signal_from_PHY((*read_tr).second, device_id);
		...
		while (write_tr != domains[stream_id]->Write_transactions_behind_LPA_barrier.end()) {
			handle_transaction_serviced_signal_from_PHY((*write_tr).second, device_id);
	...
	inline void Address_Mapping_Unit_Page_Level::Remove_barrier_for_accessing_mvpn(stream_id_type stream_id, MVPN_type mvpn)
	{
		...
		if (domains[stream_id]->MVPN_read_transactions_waiting_behind_barrier.find(mvpn) != domains[stream_id]->MVPN_read_transactions_waiting_behind_barrier.end()) {
			...
			handle_transaction_serviced_signal_from_PHY(readTR, device_id);
		...
		if (domains[stream_id]->MVPN_write_transaction_waiting_behind_barrier.find(mvpn) != domains[stream_id]->MVPN_write_transaction_waiting_behind_barrier.end()) {
			...
			MPPN_type mppn = domains[stream_id]->GlobalTranslationDirectory[mvpn].MPPN;
			NVM_Transaction_Flash_WR* writeTR = new NVM_Transaction_Flash_WR(device_id,Transaction_Source_Type::MAPPING, stream_id, SECTOR_SIZE_IN_BYTE * sector_no_per_page,
				mvpn, mppn, NULL, mvpn, NULL, (((page_status_type)0x1) << sector_no_per_page) - 1, CurrentTimeStamp(device_id));
			...
			handle_transaction_serviced_signal_from_PHY(writeTR , device_id);
