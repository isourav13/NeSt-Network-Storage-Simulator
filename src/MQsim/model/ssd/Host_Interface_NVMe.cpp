...
inline void Input_Stream_Manager_NVMe::Submission_queue_tail_pointer_update(stream_id_type stream_id, uint16_t tail_pointer_value)
{
    ...
    unsigned int qf_size_wrr = (HIGH_WRR*8)+(MED_WRR*8)+(LOW_WRR*8);
    /*if(stream_id==1)
        qf_size_wrr = 3;
    else if(stream_id==2)
        qf_size_wrr = 2;
    ...
    {
        unsigned int total_on_the_fly = 0;
        for(int i=0;i<3;i++)
            total_on_the_fly+= ((Input_Stream_NVMe *)input_streams[i])->On_the_fly_requests;
        unsigned int fetch_limit = 0;
        if(total_on_the_fly< qf_size_wrr)
        {
            switch(stream_id){
                case 1:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < HIGH_WRR*8)
                        fetch_limit = HIGH_WRR*8;
                    break;
                case 2:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < MED_WRR*8)
                        fetch_limit = MED_WRR*8;
                    break;
                case 3:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < LOW_WRR*8)
                        fetch_limit = LOW_WRR*8;
                    break;
                default:
                    fetch_limit = 0;
            }
        ...
        if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < fetch_limit)
        {
        	/*if(stream_id!=0)
        		NS_LOG_UNCOND("Not zero");*/
            ((Host_Interface_NVMe *)host_interface)->request_fetch_unit->Fetch_next_request(stream_id);
            ((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests++;
            ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head++; //Update submission queue head after starting fetch request
            if (((Input_Stream_NVMe *)input_streams[stream_id])->Submission_head == ((Input_Stream_NVMe *)input_streams[stream_id])->Submission_queue_size)
...
inline void Input_Stream_Manager_NVMe::Handle_serviced_request(User_Request *request)
{
	...
	DELETE_REQUEST_NVME(request,device_id);
}
...
void Input_Stream_Manager_NVMe::segment_user_request(User_Request *user_request)
{
	...
	while (handled_sectors_count < req_size)
	{
		...
		{
			NVM_Transaction_Flash_RD *transaction = new NVM_Transaction_Flash_RD(device_id,Transaction_Source_Type::USERIO, user_request->Stream_id,
																				 transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA, user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp(device_id));
		...
		{ //user_request->Type == UserRequestType::WRITE
			NVM_Transaction_Flash_WR *transaction = new NVM_Transaction_Flash_WR(device_id, Transaction_Source_Type::USERIO, user_request->Stream_id,
																				 transaction_size * SECTOR_SIZE_IN_BYTE, lpa, user_request, user_request->Priority_class, 0, access_status_bitmap, CurrentTimeStamp(device_id));
			user_request->Transaction_list.push_back(transaction);
...
void Request_Fetch_Unit_NVMe::Process_pcie_read_message(uint64_t address, void *payload, unsigned int payload_size)
{
	...
	case DMA_Req_Type::REQUEST_INFO:
	{
		...
		new_request->STAT_InitiationTime = MQsim_Simulator[device_id]->Time();
	...
	case DMA_Req_Type::WRITE_DATA:
		COPYDATA(((User_Request *)dma_req_item->object)->Data, payload, payload_size,device_id);
																																									   ...
																																									   submission_queue_depth(submission_queue_depth), completion_queue_depth(completion_queue_depth), no_of_input_streams(no_of_input_streams)
{
    ...
    if(std::isdigit(this->ID()[10])==0) {
        this->input_stream_manager->device_id = this->ID()[9] - '0';
        this->request_fetch_unit->device_id = this->ID()[9] - '0';
    }
    else {
        this->input_stream_manager->device_id = std::stoi(this->ID().substr(9, 2));
        this->request_fetch_unit->device_id = std::stoi(this->ID().substr(9, 2));
    }
