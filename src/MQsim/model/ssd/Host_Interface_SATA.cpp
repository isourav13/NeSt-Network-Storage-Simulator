	...
	void Input_Stream_Manager_SATA::segment_user_request(User_Request* user_request)
	{
		...
		while (handled_sectors_count < req_size) {
			...
			if (user_request->Type == UserRequestType::READ) {
				NVM_Transaction_Flash_RD* transaction = new NVM_Transaction_Flash_RD(device_id, Transaction_Source_Type::USERIO, SATA_STREAM_ID,
					transaction_size * SECTOR_SIZE_IN_BYTE, lpa, NO_PPA, user_request, 0, access_status_bitmap, CurrentTimeStamp(device_id));
				...
				NVM_Transaction_Flash_WR* transaction = new NVM_Transaction_Flash_WR(device_id, Transaction_Source_Type::USERIO, SATA_STREAM_ID,
					transaction_size * SECTOR_SIZE_IN_BYTE, lpa, user_request, 0, access_status_bitmap, CurrentTimeStamp(device_id));
				user_request->Transaction_list.push_back(transaction);
