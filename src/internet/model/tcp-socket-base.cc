																																					...
																																					.AddAttribute ("LimitedTransmit", "Enable limited transmit",
																																							...
																																							.AddAttribute ("UseEcn", "Parameter to set ECN functionality",
																																									...
																																									MakeEnumChecker (TcpSocketState::Off, "Off",
																																											...
																																											.AddAttribute ("TrafficPriority", "PriorityOfPackets",
																																													UintegerValue (0),
																																													MakeUintegerAccessor (&TcpSocketBase::m_trafficpriority),
																																													MakeUintegerChecker<uint32_t> ())
																																													.AddAttribute ("EnableDiff", "Enable differentiated Services",
																																															BooleanValue (false),
																																															MakeBooleanAccessor (&TcpSocketBase::m_enablediff),
																																															MakeBooleanChecker())
																																															.AddAttribute ("TargetThroughput", "Target Throughput Flows",
																																																	DoubleValue (1.0),
																																																	MakeDoubleAccessor(&TcpSocketBase::m_targetThroughput),
																																																	MakeDoubleChecker<double>())
																																																	.AddAttribute ("BtlBW", "BottleNeck Bandwidth",
																																																			DoubleValue (1.0),
																																																			MakeDoubleAccessor(&TcpSocketBase::m_bottleneck),
																																																			MakeDoubleChecker<double>())
																																																			.AddAttribute ("TotalBytes", "Total bytes to send",
																																																					UintegerValue (0),
																																																					MakeUintegerAccessor (&TcpSocketBase::m_totalBytes),
  ...
  m_ecnCWRSeq (sock.m_ecnCWRSeq)
{
	...
	//NS_LOG_UNCOND ("Invoked the copy constructor");

	//NS_LOG_UNCOND(m_maxWinSize);
...
{
	//NS_LOG_UNCOND(m_maxWinSize);
	if(m_tcb->m_targetThroughput != m_targetThroughput)
	{m_tcb->m_targetThroughput = m_targetThroughput;
	}
	//m_tcb->m_targetThroughput_initial = m_targetThroughput * m_bottleneck;}
...
TcpSocketBase::DoConnect (void)
{
	...
	m_tcb-> m_btl_bw = m_bottleneck;
	m_tcb->m_quality_factor = 1.0;

	//NS_LOG_UNCOND("BottleNeck Bandwidth "<<m_bottleneck);
...
TcpSocketBase::ReceivedAck (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
	...
	m_tcb->m_trpriority = m_trafficpriority;
	m_tcb->m_enablediff = m_enablediff;
		...
		uint32_t currentDelivered, const SequenceNumber32 &oldHeadSequence)
{
	...
	else if (ackNumber > oldHeadSequence)
	{
		...
		{
			if (m_tcb->m_congState == TcpSocketState::CA_OPEN) //works most of the time
			{
...
TcpSocketBase::SendDataPacket (SequenceNumber32 seq, uint32_t maxSize, bool withAck)
{
	...
	if (m_tcb->m_trpriority == 0)
	{
		//NS_LOG_UNCOND("The maxsize "<<maxSize<<" "<<m_tcb->m_cWnd);
	}
...
TcpSocketBase::EstimateRtt (const TcpHeader& tcpHeader)
{
	...
	//NS_LOG_UNCOND(" In EstimateRtt "<<m_tcb->m_lastRtt);
	...
	if (!m_history.empty ())
	{
		...
		{ // Ok to use this sample
			...
			{
				//NS_LOG_UNCOND("TCP header has TS option");
				...
				//NS_LOG_UNCOND("Echo value m value "<<m.GetMicroSeconds());
	...
	if (!m.IsZero ())
	{
		//		NS_LOG_UNCOND("RTT Estimate "<<m_rtt->GetEstimate ().GetMicroSeconds()<<" RTT Variance "<<m_rtt->GetVariation ()<<" App id "<<m_tcb->m_trpriority);
		//
		//
		//
		//		uint32_t p_90 = m_rtt->GetEstimate ().GetMicroSeconds() + (m_rtt->GetVariation ().GetNanoSeconds()* 1.05)/1000.0;
		//
		//		NS_LOG_UNCOND("p_90 "<<p_90);
