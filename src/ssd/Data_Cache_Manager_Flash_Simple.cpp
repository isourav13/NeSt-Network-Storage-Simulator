	...
	void Data_Cache_Manager_Flash_Simple::process_new_user_request(User_Request* user_request)
	{
		...
		if (user_request->Type == UserRequestType::READ) {
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				default:
					PRINT_ERROR("The specified caching mode is not not support in simple cache manager! 1")
			}
			...
			switch (caching_mode_per_input_stream[user_request->Stream_id])
			{
				...
				default:
					PRINT_ERROR("The specified caching mode is not not support in simple cache manager! 2")
			}
		}
	}
		...
		if (transaction->Type == Transaction_Type::READ) {
			...
			switch (Data_Cache_Manager_Flash_Simple::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				case Caching_Mode::WRITE_CACHE:
					...
					break;
				default:
					PRINT_ERROR("The specified caching mode is not not support in simple cache manager! 3")
			}
		} else { //This is a write request
			switch (Data_Cache_Manager_Flash_Simple::caching_mode_per_input_stream[transaction->Stream_id])
			{
				...
				default:
					PRINT_ERROR("The specified caching mode is not not support in simple cache manager! 4")
			}
		}
	}
