...
{
	//StorageType storage_type; //SD Changes
	Data_Cache_Flash::Data_Cache_Flash(unsigned int capacity_in_pages) : capacity_in_pages(capacity_in_pages) {
		//storage_type = StorageType::PERSISTENT_MEMORY; //SD Changes
	}

    void Data_Cache_Flash::Warmup(const stream_id_type stream_id)
    {
        for(uint64_t lpn = 1;lpn<capacity_in_pages;lpn++) {
            ...
            cache_slot->Content = 1;
            cache_slot->Timestamp = 0;
	...
	bool Data_Cache_Flash::Exists(const stream_id_type stream_id, const LPA_type lpn)
	{
		...
		if (it == slots.end()) { //JG Changes
			return false;
		}

		//SD Changes
	/*	if (it == slots.end() && storage_type == StorageType::PERSISTENT_MEMORY) {
		        return false;
		    }*/
		return true;
	}
