...
inline void Input_Stream_Manager_NVMe::Submission_queue_tail_pointer_update(stream_id_type stream_id, uint16_t tail_pointer_value)
{
    ...
    unsigned int qf_size_wrr = (HIGH_WRR)+(MED_WRR)+(LOW_WRR);
    /*if(stream_id==1)
        qf_size_wrr = 3;
    else if(stream_id==2)
        qf_size_wrr = 2;
    ...
    else
    {
        ...
        if(total_on_the_fly< qf_size_wrr)
        {
            switch(stream_id){
                case 1:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < HIGH_WRR)
                        fetch_limit = HIGH_WRR ;
                    break;
                case 2:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < MED_WRR)
                        fetch_limit = MED_WRR ;
                    break;
                case 3:
                    if(((Input_Stream_NVMe *)input_streams[stream_id])->On_the_fly_requests < LOW_WRR)
                        fetch_limit = LOW_WRR;
                    break;
                default:
                    fetch_limit = 0;
