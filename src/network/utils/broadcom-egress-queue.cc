...
namespace ns3 {
		...
		BroadcomQueue()
	{
		...
		{
			m_bytesInQueue[i] = 0;
			m_queues.push_back(CreateObject<BroadcomDropTailQueue>());
		}
		...
		BEgressQueue::DoDequeueRR(bool paused[]) //this is for switch only
	{
		...
		if (found)
		{
			...
			if (Simulator::Now() > m_bwsatisfied[qIndex])
	...
	void
		BEgressQueue::RecoverQueue(Ptr<BroadcomDropTailQueue> buffer, uint32_t i)
	{
		Ptr<Packet> packet;
		Ptr<BroadcomDropTailQueue> tmp = CreateObject<BroadcomDropTailQueue>();
