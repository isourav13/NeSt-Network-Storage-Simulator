...
namespace ns3 {
		...
		QbbNetDevice::Receive(Ptr<Packet> packet)
	{
		...
		else if (ipv4h.GetProtocol() == 0xFD)//NACK on NIC
		{
			...
			for (i = 1; i < m_queue->m_fcount; i++)
			{
				if (m_findex_udpport_map[i] == (uint32_t)port && m_findex_qindex_map[i] == (uint32_t)qIndex)
				{
					break;
				}
			}
			...
			if (!m_backto0)
			{
				if (buffer_seq > (uint32_t)seq)
				{
					std::cout << "ERROR: Sendingbuffer miss!\n";
				}
				while ((uint32_t)seq > buffer_seq)
				{
		...
		else if (ipv4h.GetProtocol() == 0xFC)//ACK on NIC
		{
			...
			if (m_waitAck && (uint32_t)seq >= m_milestone_tx[i])
			{
			...
			else
			{
				...
				for (i = 1; i < fCnt; i++)
				{
					if (m_findex_udpport_map[i] == 0)
					{
						...
						m_sendingBuffer[i] = CreateObject<BroadcomDropTailQueue>();
						if (m_waitAck)
						{
							m_milestone_tx[i] = m_chunk;
						}
		...
		QbbNetDevice::ReceiverCheckSeq(uint32_t seq, uint32_t key)
	{
		...
		if (seq == expected)
		{
			...
			if (ReceiverNextExpectedSeq[key] > (uint32_t)m_milestone_rx[key])
			{
