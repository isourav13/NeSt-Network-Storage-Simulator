																												...
																												enabled_logging(enabled_logging), logging_period(logging_period), logging_file_path(logging_file_path)
{
		...
		switch (SSD_device_type) {
			case HostInterface_Types::NVME:
				...
				if (NVME_SQ_FULL(nvme_queue_pair) || available_command_ids.size() == 0) {
					...
					pcie_root_complex->Write_to_device(nvme_queue_pair.Submission_tail_register_address_on_device, nvme_queue_pair.Submission_queue_tail, PCIe_Destination_Type::DEVICE_RW);//Based on NVMe protocol definition, the updated tail pointer should be informed to the device
				}
				break;
			case HostInterface_Types::SATA:
	...
	uint32_t IO_Flow_Base::Get_device_response_time()
	{
		...
		return (uint32_t)(STAT_sum_device_response_time / STAT_serviced_request_count );
