...
#include "../ssd/Host_Interface_NVMe_Priorities.h"
...
namespace Host_Components
{
	...
	private:
		...
		int packet_overhead;
		std::queue<PCIe_Message*> Message_buffer_toward_ssd_device_b[IO_Flow_Priority_Class::NUMBER_OF_PRIORITY_LEVELS];
        std::queue<PCIe_Message*> Message_buffer_toward_ssd_device_a;//[IO_Flow_Priority_Class::NUMBER_OF_PRIORITY_LEVELS + 1];
        std::queue<PCIe_Message*> Message_buffer_toward_ssd_device;
        std::queue<PCIe_Message*> Message_buffer_toward_ssd_device_compq;
        int weight_before_scheduling[IO_Flow_Priority_Class::NUMBER_OF_PRIORITY_LEVELS];
        int weight_after_scheduling[IO_Flow_Priority_Class::NUMBER_OF_PRIORITY_LEVELS];
        int robin_count;
        int ios;
